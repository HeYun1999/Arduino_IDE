...
// 4. 广播数据缓冲区
uint8_t adv_data[31] = {0};  // BLE广播包最大31字节
...
void updateAdvertisingData() {
  // 构建厂商自定义数据
  // 格式: [长度(1字节)][类型(0xFF=厂商自定义)][厂商ID(2字节)][自定义数据(N字节)]
  uint8_t data_len = 0;
  
  // 厂商ID (2字节，小端序)
  adv_data[2] = (MANUFACTURER_ID >> 0) & 0xFF;  // 低8位
  adv_data[3] = (MANUFACTURER_ID >> 8) & 0xFF;  // 高8位
  data_len += 2;
  
  // 温度数据 (4字节，float类型)
  memcpy(&adv_data[4], &customData.temperature, 4);
  data_len += 4;
  
  // 电池电量 (1字节)
  adv_data[8] = customData.battery;
  data_len += 1;
  
  // 计数器 (2字节)
  adv_data[9] = (customData.counter >> 0) & 0xFF;  // 低8位
  adv_data[10] = (customData.counter >> 8) & 0xFF; // 高8位
  data_len += 2;
  
  // 设置数据长度和类型
  adv_data[0] = 2 + data_len;  // 总长度 = 类型(1) + 厂商ID(2) + 数据长度
  adv_data[1] = 0xFF;          // 类型：厂商自定义数据
  
  // 更新广播数据
  Bluefruit.Advertising.clearData();
  Bluefruit.Advertising.addData(adv_data, adv_data[0] + 1);  // +1 包含长度字节
}
